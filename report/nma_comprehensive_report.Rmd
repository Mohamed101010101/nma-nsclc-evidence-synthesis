...
# Executive Summary & Philosophical Hook {.tabset}
...
- **Diagonal:** Treatments ordered by P-score from top-left (best) to bottom-right (worst).
- **Lower Triangle:** Network Meta-Analysis estimates ($\text{HR} [95\% \text{CI}]$) combining all direct and indirect paths.
- **Upper Triangle:** Direct pairwise comparisons. Dots (`.`) indicate treatment pairs with **zero direct head-to-head clinical trials**, proving how network meta-analysis informs clinical decisions through indirect evidence.

```{r table_league, echo=FALSE}
lg_mat <- read.csv("../outputs/tables/league_table_random_common.csv", row.names = 1)
knitr::kable(lg_mat, caption = "Table: Complete League Table Matrix")
```
...
## Net Heat Plot (Inconsistency Hot-Spots)
The Net Heat plot identifies which specific trial designs drive instability or introduce tension into the network estimates.

```{r fig_netheat, echo=FALSE, out.width="90%"}
